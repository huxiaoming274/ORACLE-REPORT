...
def test_connection(config_file):
    ...
    try:
        # 使用瘦模式连接
        connection = cx_Oracle.connect(user=user, password=password, dsn=dsn, mode=cx_Oracle.SYSDBA)
        connection.close()
...
def select_sql_file():
    file_path = filedialog.askopenfilename(title="选择SQL文件 / Select SQL File", filetypes=[("SQL Files", "*.sql")])
    sql_file_var.set(file_path)

def select_output_file():
    file_path = filedialog.asksaveasfilename(defaultextension=".xlsx", title="保存为 / Save As", filetypes=[("Excel Files", "*.xlsx")])
...
def export_data_to_excel(config_file, sql_file, output_file):
    ...
    try:
        ...
        messagebox.showinfo("成功 / Success", f"数据已成功导出到 {output_file} / Data has been successfully exported to {output_file}")
...
# 添加测试连接按钮
tk.Button(root, text="测试连接 / Test Connection", command=lambda: test_connection('config.ini')).grid(row=4, column=0, columnspan=3, pady=20)
